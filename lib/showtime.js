(function () {
  ...
  pgClient.query("CREATE TABLE IF NOT EXISTS omdb (id serial NOT NULL, omdb json, CONSTRAINT omdb_pk PRIMARY KEY (id));", [], function (error, result) {
    if (error === null) {
      pgClient.query('SELECT omdb FROM omdb WHERE id=$1;', [1], function (error, result) {
        if (error === null && result.rowCount === 1) {
          omdbCache = result.rows[0].omdb;
          ...
          _.each(omdbCache, function (movieInfo, movieTitle, list) {
            if (movieInfo !== null) {
          ...
          pgClient.query('INSERT INTO omdb (id, omdb) VALUES ($1, $2);', [1, {"": null}], function (error, result) {});
        }
      });
    }
  });
      ...
      scraperjs.StaticScraper
        ...
        .scrape(function ($) {
          ...
          async.eachSeries([0, 1, 2, 3, 4, 5, 6], function (dindex, dcallback) {
            ...
            lastScraped = moment();
            pgClient.query('UPDATE omdb SET omdb=$1 WHERE id=$2;', [omdbCache, 1], function (error, result) {});
            callback(showtime);
          });
        });
    } else {
