  ...
  return () => new Promise((resolve, reject) => {
    request
      ...
      .then((res) => {
        ...
        // checking with detail cache table and building new cache list...
        Promise
          .all(moviesDetailKeys.map(detail => detailCache(moedoo, detail)))
          .then((detailCacheData) => {
            const newDetailData = [];
            const newDetailDataPromise = [];

            detailCacheData.forEach((detailCacheDataDetail, index) => {
              if (detailCacheDataDetail === 'ø') {
                console.log(`[caching] ${moviesDetailKeys[index]}`);
                newDetailData.push(moviesDetailKeys[index]);
                newDetailDataPromise.push(request('GET', moviesDetailKeys[index]));
              } else {
                console.log(`[cached] ${moviesDetailKeys[index]}`);
                moviesDetail[moviesDetailKeys[index]] = detailCacheDataDetail;
              }
            });

            // requesting new detail pages...
            Promise
              .all(newDetailDataPromise)
              .then((moviesDetailKeysData) => {
                moviesDetailKeysData.forEach((moviesDetailKeyResponse, index) => {
                  ...
                  newDetailData[newDetailData[index]] = youtube.length > 0 ? youtube.attr('src') : null;

                  // checking imdb embed if YouTube is not found...
                  if (newDetailData[newDetailData[index]] === null) {
                    newDetailData[newDetailData[index]] = imdb.length > 0 ? imdb.attr('src') : null;
                  }

                  // storing new detail info in the detail table...
                  detailCache(moedoo, newDetailData[index], moviesDetail[newDetailData[index]]);
                });
