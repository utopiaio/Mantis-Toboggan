...
module.exports = moedoo => () => new Promise((resolve, reject) => {
  request
    ...
    .then((res) => {
      ...
      const moviesPoster = Object.create(null); // this will hold url for posters
      ...
      const [c3, c2, c1] = $('body section#main div.row').splice(0, 3).map(cinema =>
        $('div', cinema).splice(1).map((div) => {
          ...
          const poster = $('img', div).attr('src');

          moviesDetail[detail] = detail;
          moviesPoster[poster] = poster;
        ...
        .then((moviesDetailKeysData) => {
          ...
          Promise
            ...
            .then((moviesTitleKeysData) => {
              ...
              const moviesPosterKeys = Object.keys(moviesPoster);
              Promise.all(moviesPosterKeys.map(moviePosterUrl => posterCache(moedoo, moviePosterUrl)))
                .then((base64Posters) => {
                  base64Posters.forEach((base64Poster, index) => {
                    moviesPoster[moviesPosterKeys[index]] = base64Poster;
                  });
