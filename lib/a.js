...
const { gregorianToEthiopic, gregorianWeekdayToEthiopicWeekday, ethiopicMonthToFullEthiopicMonth } = require('./ec');
...
module.exports = moedoo => () => new Promise((resolve, reject) => {
  request
    ...
    .then((res) => {
      ...
      /**
       * using $.map is returning *additional properties*
       * know why the following code returns whole lot of list rather than just numbers
       * const cinemas = $('header > div.row').map((index, cinema) => index);
       * // { '0': 0, '1': 1, '2': 2, options... }
       */
      const cinemas = { 0: [], 1: [], 2: [] };

      $('header > div.row').each((index, cinema) => {
        $('div.col-md-2', cinema).each((_, movie) => {
          const detail = `${config.EDNA_URL}${$('a', movie).attr('href')}`;
      ...
      // TODO: figure out why mapping is returning extra properties and go back to mapping
      const [c3, c2, c1] = [cinemas[0], cinemas[1], cinemas[2]];
            ...
            .then((moviesTitleKeysData) => {
              ...
              resolve({
                ...
                meta: {
                  ...
                  ec: `${gregorianWeekdayToEthiopicWeekday(gc.format('dddd'))}, ${ethiopicMonthToFullEthiopicMonth(ec.month)} ${geez(ec.day)}`, // dddd, MMMM D
                },
              });
            });
        });
