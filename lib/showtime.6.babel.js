      ...
      _async2['default'].forEachOfSeries(dayRunner, function (dayValue, dayKey, dayCallback) {
        ...
        _async2['default'].eachSeries([0, 1, 2, 3, 4], function (movie, movieCallback) {
          ...
          getMovieInfo(cinema1Movie).then(function (info1) {
            showtime[dayKey]['C1'].push({ time: cinema1Showtime, movie: cinema1Movie, info: info1 });

            getMovieInfo(cinema2Movie).then(function (info2) {
              showtime[dayKey]['C2'].push({ time: cinema2Showtime, movie: cinema2Movie, info: info2 });

              getMovieInfo(cinema3Movie).then(function (info3) {
                showtime[dayKey]['C3'].push({ time: cinema3Showtime, movie: cinema3Movie, info: info3 });

                movieCallback(null);
              }, function (error3) {
                showtime[dayKey]['C3'].push({ time: cinema3Showtime, movie: cinema3Movie, info: null });
              });
            }, function (error2) {
              showtime[dayKey]['C2'].push({ time: cinema2Showtime, movie: cinema2Movie, info: null });
            });
          }, function (error1) {
            showtime[dayKey]['C1'].push({ time: cinema1Showtime, movie: cinema1Movie, info: null });
          });
        }, function (movieError) {
